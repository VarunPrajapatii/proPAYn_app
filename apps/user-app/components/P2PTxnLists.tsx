export default function P2pTxnLists({
    transaction
} : {
    transaction : {
        user_no: string,
                ...
                <div>
                    <div className="flex justify-between sm:px-1">
                        <div className="flex p-1 sm:p-3">
                            <div className="p-1 sm:p-4 my-auto">
                                <div className="">
                                    <svg
                                        ...
                                        className="w-9 sm:w-16 fill-current dark:text-blue-700 text-customBlue-mid"
                                    >
                            ...
                            <div className="pt-1">
                                <div className="font-nunito font-light text-xs sm:text-md">
                                    {(transaction.transactionType == "Credit") ? "Credit to Wallet" : "Debited from Wallet"}
                                </div>
                                <div className="font-poppins  text-sm sm:text-xl">
                                    {(transaction.transactionType === "Credit") ? "From": "Sent To"}: +91 {transaction.relatedUser_no}
                                </div>
                                <div className="font-nunito text-xs sm:text-base dark:text-gray-400 text-gray-700">
                                    {transaction.time.toDateString()}
                                </div>
                            </div>
                        </div>
                        <div className={`px-3 py-5 sm:px-8 sm:py-10 text-lg sm:text-xl font-jetbrains font-bold ${(transaction.transactionType === "Credit") ? "text-green-600" : "text-red-600"}`}>
                            {(transaction.transactionType === "Credit") ? "+" : "-"}
                            <span className='text-xs sm:text-base'>₹</span>
                            {Math.abs(transaction.amount / 100)}
                        </div>
                    </div>
                    <div className="border border-y-gray-400 mx-4 sm:mx-8">
