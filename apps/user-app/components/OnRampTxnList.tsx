        ...
        <div>
            <div className="flex justify-between sm:px-1">
                <div className="flex p-1 sm:p-3">
                    <div className="p-1 sm:p-4 my-auto">
                        <div className="">
                            <svg
                                ...
                                className="w-9 sm:w-12 fill-current dark:text-blue-700 text-customBlue-mid" // Example Tailwind CSS classes
                            >
                    ...
                    <div className="pt-1">
                        <div className="font-poppins sm:font-bold text-sm sm:text-lg">
                            Added to <span className="hidden sm:block">proPAYn</span> wallet
                        </div >
                        {
                            (transaction.status === "Processing") 
                            ? 
                                (<div className="font-nunito font-extralight text-xs sm:text-sm">From: {transaction.provider} 
                                    <span className="font-nunito font-semibold text-yellow-600">(Processing)</span>
                                </div>) 
                            : 
                                ((transaction.status === "Failure")
                                ?
                                    (<div className="font-nunito font-base text-xs sm:text-sm">From: {transaction.provider} <span className="font-nunito font-semibold text-red-600">(Failure)</span></div>)
                                :
                                    (<div className="font-nunito font-base text-xs sm:text-sm">From: {transaction.provider} <span className="font-nunito font-semibold text-green-600">(Success)</span></div>)
                                )
                        }
                        
                        <div className="font-nunito text-sm sm:text-base dark:text-gray-400 text-gray-700">
                            {transaction.time.toDateString()}
                        </div>
                    </div>
                </div>
                <div className="px-5 py-7 text-xl sm:text-2xl font-jetbrains font-semibold">
                    <span className='text-sm sm:text-base'>₹{""}</span>{transaction.amount / 100}
                </div>
            </div>
